...
namespace CapaPresentacion
{
    public partial class FrmTypePayment : Form
    {
        ...
        {
            InitializeComponent();
            this.ttMensaje.SetToolTip(this.txtNombre, "Ingrese el Nombre del Tipo de Pago");
        }
        ...
        private void Mostar()
        {
            this.dataListado.DataSource = NTypePayment.Show();
        ...
        {
            this.dataListado.DataSource = NTypePayment.TextSearch(this.txtBuscar.Text);
            this.OcultarColumnas();
            lbltotal.Text = "Total de Registros: " + Convert.ToString(dataListado.Rows.Count);
        }

        private void FrmTypePayment_Load(object sender, EventArgs e)
        {
            this.Top = 0;
            this.Left = 0;
            this.Mostar();
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        rpta = NTypePayment.Insert(this.txtNombre.Text.Trim().ToUpper());
                    }
                    else
                    {
                        rpta = NTypePayment.update(Convert.ToInt32(this.txtId.Text), this.txtNombre.Text.Trim().ToUpper());
                    }
                    if (rpta.Equals("OK"))
                    {
                        if (this.IsNuevo)
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = NTypePayment.Delete(Convert.ToInt32(Codigo));
